...
Vectors describe quantities with one direction slot, while \term{tensors} describe multilinear relations between vectors and covectors. In index notation, a scalar has no index, a vector has one index, and a rank-two tensor has two indices:
\begin{equation}
\text{scalar: }\phi,
\qquad
\text{vector: }v_i,
...
w_i=C_{ijk}u_jv_k.
\end{equation}
This operation is linear in $u_j$ when $v_k$ is fixed, and linear in $v_k$ when $u_j$ is fixed; this is the meaning of \term{multilinear}.
...
\begin{figure}[htb]
    \centering
    \subimport{tikz/}{traction.tex}
    \caption{Traction on an oriented surface element. In an anisotropic medium $\mathbf{t}=\sigma\hat{\mathbf{n}}$ need not be parallel to the geometric normal.}
    \label{fig:vec-traction}
\end{figure}
...
\begin{figure}[htb]
    \centering
    \subimport{tikz/}{inertia.tex}
    \caption{The inertia tensor maps $\boldsymbol{\omega}$ to $\mathbf{L}=I\boldsymbol{\omega}$. The two vectors are parallel only in a principal-axis frame, or for a sufficiently symmetric body.}
    \label{fig:vec-inertia}
\end{figure}
...
\begin{figure}[htb]
    \centering
    \subimport{tikz/}{tensor_network.tex}
    \caption{Matrix multiplication as a tensor network: the contracted index $k$ is an internal bond.}
    \label{fig:vec-tensor-network}
\end{figure}
