...
\end{equation}
The symbol $\nabla$ has no standalone meaning; combined with a scalar field, a dot product, or a cross product, it produces the \term{gradient}, \term{divergence}, or \term{curl} respectively.
...
\begin{figure}[htb]
    \centering
    \subimport{tikz/}{gradient.tex}
    \caption{Level curves of a scalar field and the gradient $\nabla f$, which is orthogonal to each equipotential and points toward increasing $f$.}
    \label{fig:vec-gradient}
\end{figure}
...
\begin{figure}[htb]
    \centering
    \subimport{tikz/}{curvilinear.tex}
    \caption{Local orthonormal bases in cylindrical and spherical coordinates, in which the gradient formulae of this section are written.}
    \label{fig:vec-curvilinear}
\end{figure}
...
\begin{figure}[htb]
    \centering
    \subimport{tikz/}{path_independence.tex}
    \caption{Two paths from the origin to $P=(x_0,y_0)$. For $\mathbf{F}=\nabla f$ the line integrals coincide.}
    \label{fig:vec-path}
\end{figure}
...
\end{equation}
Thus divergence is the local rate of net outward flux per unit volume. The \term{divergence theorem} relates the local and global descriptions:
\begin{equation}
...
\begin{figure}[htb]
    \centering
    \subimport{tikz/}{divergence.tex}
    \caption{Net outward flux through a small rectangular volume. Dividing by $\Delta V$ produces $\nabla\cdot\mathbf{A}$ in the continuum limit.}
    \label{fig:vec-divergence}
\end{figure}
...
\begin{figure}[H]
    \centering
    \subimport{tikz/}{curl_loop.tex}
    \caption{Circulation around a small rectangle in the $xy$-plane. The enclosed flux of $\nabla\times\mathbf{A}$ is $(\nabla\times\mathbf{A})_z\,\Delta x\,\Delta y$.}
    \label{fig:vec-curl}
\end{figure}
More generally,
\begin{equation}
...
\termidx{Stokes' theorem}{Stokes' theorem} follows by dividing a surface into small loops. Integrals on internal edges cancel because neighboring loops traverse each shared edge in opposite directions:
\begin{equation}
...
\begin{figure}[htb]
    \centering
    \subimport{tikz/}{stokes.tex}
    \caption{The Stokes example: $\mathbf{A}=-y\,\mathbf{e}_x+x\,\mathbf{e}_y-z\,\mathbf{e}_z$ has counterclockwise circulation on the circle $L$, matching $\nabla\times\mathbf{A}=2\mathbf{e}_z$ through the disk $S$.}
    \label{fig:vec-stokes}
\end{figure}
