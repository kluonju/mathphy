...
\renewcommand{\currentchapterpath}{chapters/vec/}
...
A \term{vector} is a mathematical object that has both magnitude and direction,
which distinguishes it from a \term{scalar}, which has only magnitude.
...
Linear maps between such spaces are represented by matrices; when
$A\mathbf{x}=\mathbf{b}$ is invertible, the original vector is recovered by
$\mathbf{x}=A^{-1}\mathbf{b}$, constructed explicitly in the next sections.
Standard accounts of the same material are given by Arfken, Weber, and Harris~\cite{arfken} and by Byron and Fuller~\cite{byron}.


\chapterinput{basics}
\chapterinput{matrices}
