...
For a nonperiodic function on the real line, regard the Fourier series of the previous chapter as the limit $\ell\to\infty$. With $\omega_n=n\pi/\ell$ and $\Delta\omega=\pi/\ell$, the discrete frequencies become continuous. The original source keeps the limiting idea explicit:
\begin{equation}
...
\end{equation}
As $\ell\to\infty$, the sum turns into an integral, and the coefficient becomes frequency density. This is the essence of the \term{Fourier integral}. Using the symmetric convention, the \term{Fourier transform} and its inverse are
\begin{align}
...
\end{equation}
where $A(\omega)=\pi^{-1}\int f(x)\cos(\omega x)\,\mathrm{d}x$ and $B(\omega)=\pi^{-1}\int f(x)\sin(\omega x)\,\mathrm{d}x$. The representation $C(\omega)\cos[\omega x-\varphi(\omega)]$ defines the amplitude spectrum $C=(A^2+B^2)^{1/2}$ and phase spectrum $\varphi=\arctan(B/A)$.

\begin{example}
For $f(x)=e^{-\alpha|x|}$ with $\alpha>0$, we split the integral at $x=0$ and obtain
\begin{align}
F(\omega) &=\sqrt{\frac{1}{2\pi}}\left[\int_{-\infty}^{0}e^{\alpha x}e^{-\mathrm{i}\omega x}\,\mathrm{d}x+
\int_0^{\infty}e^{-\alpha x}e^{-\mathrm{i}\omega x}\,\mathrm{d}x\right]\nonumber\\
...
\mathcal{F}\left[\int^x f(s)\,\mathrm{d}s\right]&=\frac{F(\omega)}{\mathrm{i}\omega},\\
...
\end{align}
The derivative theorem is especially important: differentiation in physical space corresponds to multiplication by $\mathrm{i}\omega$ in frequency space. The same idea converts many linear \acrodef{ODE}{ordinary differential equations} and PDEs into algebraic problems in Fourier space. For \term{convolution} $(f_1*f_2)(x)=\int_{-\infty}^{\infty}f_1(s)f_2(x-s)\,\mathrm{d}s$, the symmetric normalization gives $\mathcal{F}[f_1*f_2]=\sqrt{2\pi}F_1F_2$.

\subsection{Higher-dimensional transforms}

For $f(\mathbf{r})$ on $\mathbb{R}^d$,
\begin{align}
F(\mathbf{k})&=\frac{1}{(2\pi)^{d/2}}\int_{\mathbb{R}^d}f(\mathbf{r})e^{-\mathrm{i}\mathbf{k}\cdot\mathbf{r}}\,\mathrm{d}^d\mathbf{r},\\
f(\mathbf{r})&=\frac{1}{(2\pi)^{d/2}}\int_{\mathbb{R}^d}F(\mathbf{k})e^{\mathrm{i}\mathbf{k}\cdot\mathbf{r}}\,\mathrm{d}^d\mathbf{k}.
\end{align}
