...
\end{equation}
The second identity is the \term{sifting property}: the delta function picks out the value of the function at the singular point. It is the defining property; one does not assign numerical values to $\delta(x)$ itself, only to its integrals against smooth test functions.
...
\end{equation}
which is the statement that the frequency spectrum of an ideal point source is flat. With the symmetric convention used throughout these notes,
$F(k)=(2\pi)^{-1/2}\int f(x)e^{-\mathrm{i}kx}\,\mathrm{d}x$, one has $\mathcal{F}[\delta(x)]=1/\sqrt{2\pi}$.
...
\begin{itemize}
  ...
  \item \textbf{Finite interval.} $\displaystyle\int_a^b f(x)\delta(x-x_0)\,\mathrm{d}x=
...
&=\int_{-\infty}^{\infty}g(x-t)f(t)\,\mathrm{d}t=g*f(x).
\end{align}

The convolution theorem connects convolution in physical space to multiplication in frequency space. Writing the inverse Fourier transform of $f$ as $f(x)=(2\pi)^{-1/2}\int e^{\mathrm{i}kx}F(k)\,\mathrm{d}k$ and exchanging orders of integration,
\begin{align}
f*g(x)&=\int f(x-y)g(y)\,\mathrm{d}y\\
&=\frac{1}{\sqrt{2\pi}}\int e^{\mathrm{i}k(x-y)}F(k)\,\mathrm{d}k\,g(y)\,\mathrm{d}y\\
&=\frac{1}{\sqrt{2\pi}}\int e^{\mathrm{i}kx}F(k)\!\left[\int e^{-\mathrm{i}ky}g(y)\,\mathrm{d}y\right]\,\mathrm{d}k\\
&=\int e^{\mathrm{i}kx}F(k)G(k)\,\mathrm{d}k,
\end{align}
which is the inverse Fourier transform of $\sqrt{2\pi}\,F(k)G(k)$. Equivalently,
\begin{equation}
\mathcal{F}[f*g]=\sqrt{2\pi}\,F(k)\,G(k),
\qquad
\mathcal{F}[fg]=\frac{1}{\sqrt{2\pi}}\,(F*G)(k).
\end{equation}
A product in real space is therefore a convolution in $k$-space, and vice versa. The result extends verbatim to $\mathbb{R}^n$ with $(2\pi)^{n/2}$ in the convolution identity and $(2\pi)^{-n/2}$ in the product identity.
...
\begin{equation}
(f*g)(x)=\int_{-b}^{b}f(x-y)\,\mathrm{d}y=\int_{x-b}^{x+b}f(t)\,\mathrm{d}t,
\end{equation}
...
\end{equation}
the convolution theorem gives $F(k)=(2\pi)^{-1/2}e^{-\sigma_1^2k^2/2}$ and $G(k)=(2\pi)^{-1/2}e^{-\mathrm{i}kx_0-\sigma_2^2k^2/2}$, so
\begin{equation}
\sqrt{2\pi}\,F(k)G(k)=\frac{1}{\sqrt{2\pi}}e^{-\mathrm{i}kx_0-(\sigma_1^2+\sigma_2^2)k^2/2}.
\end{equation}
...
\begin{equation}
f*\delta(\cdot-x_0)=\int f(x-y)\delta(y-x_0)\,\mathrm{d}y=f(x-x_0).
\end{equation}
...
\end{equation}
which is the distributional form of the replication principle. Equivalently, in the Fourier domain $\mathcal{F}[\delta(x-x_i)]=(2\pi)^{-1/2}e^{-\mathrm{i}kx_i}$, so multiplication by these phase factors corresponds to shifts; the convolution theorem then reduces the sum-of-copies identity to $\mathcal{F}[\sum_i f(x-x_i)]=F(k)\sum_i e^{-\mathrm{i}kx_i}$.
...
\begin{equation}
\int_{U_r}g(x)\,\delta(f(x))\,\mathrm{d}x
=\int_V g(\varphi(y))\,\delta(y)\,\frac{1}{|f'(\varphi(y))|}\,\mathrm{d}y
=\frac{g(x_r)}{|f'(x_r)|},
\end{equation}
...
Evaluate $I=\displaystyle\int_{-\infty}^{\infty}\tan\!\bigl(\tfrac{\pi x}{4}\bigr)\,\delta(x^4-1)\,\mathrm{d}x$.
...
Show that the integration measure $p^{-2}\,\mathrm{d}^3 p$ is covariant under Lorentz transformations. Use $\delta(E^2-p^2c^2-m^2c^4)=\delta((E-pc)(E+pc))/(2(E+pc))$. With $E>0$ this picks out the mass shell and the measure becomes $\dfrac{\mathrm{d}^3 p}{2E}$, which is invariant.
\end{example}
...
\qquad
\int_{\mathbb{R}^n}f(\mathbf{x})\,\delta(\mathbf{x}-\mathbf{y})\,\mathrm{d}^n\mathbf{x}=f(\mathbf{y}),
\end{equation}
with Fourier representation
\begin{equation}
\delta(\mathbf{x}-\mathbf{y})=\frac{1}{(2\pi)^n}\int_{\mathbb{R}^n}e^{\mathrm{i}\mathbf{k}\cdot(\mathbf{x}-\mathbf{y})}\,\mathrm{d}^n\mathbf{k}.
\end{equation}
...
\begin{equation}
\mathcal{F}\!\left[\mathop{\mathrm{III}}\nolimits_P\right](k)=\frac{\sqrt{2\pi}}{P}\,\mathop{\mathrm{III}}\nolimits_{2\pi/P}(k),
\end{equation}
...
\begin{equation}
\sum_{n=-\infty}^{\infty}f(nP)=\frac{\sqrt{2\pi}}{P}\sum_{n=-\infty}^{\infty}F\!\left(\frac{2\pi n}{P}\right),
\end{equation}
...
For the first-order equation $u'(x)+\gamma u(x)=f(x)$ with $\gamma>0$, taking the Fourier transform turns it into the algebraic equation $(\mathrm{i}k+\gamma)U(k)=F(k)$, so $\widehat{G}(k)=1/(\mathrm{i}k+\gamma)$ and
\begin{equation}
G(x)=
\begin{cases}
e^{-\gamma x}, & x>0,\\
...
\end{cases}
\qquad
u(x)=\int_{-\infty}^{x}e^{-\gamma(x-y)}f(y)\,\mathrm{d}y.
\end{equation}
...
\begin{equation}
(Q^2+k^2)\widehat{\Psi}(k)=\frac{\lambda}{\sqrt{2\pi}}\,\Psi(0),
\end{equation}
so $\widehat{\Psi}(k)=\dfrac{\lambda\,\Psi(0)}{\sqrt{2\pi}\,(Q^2+k^2)}$, and inverse-transforming with $\mathcal{F}^{-1}[1/(Q^2+k^2)]=\sqrt{\pi/(2Q^2)}\,e^{-Q|x|}$ gives
\begin{equation}
...
Compute $\mathcal{F}[\cos(qx)]$ using the sifting property. With the symmetric convention $F(k)=(2\pi)^{-1/2}\int e^{-\mathrm{i}kx}f(x)\,\mathrm{d}x$ and writing $\cos(qx)=\tfrac12(e^{\mathrm{i}qx}+e^{-\mathrm{i}qx})$,
\begin{align}
F(k)&=\frac{1}{2\sqrt{2\pi}}\int e^{-\mathrm{i}kx}e^{\mathrm{i}qx}\,\mathrm{d}x+\frac{1}{2\sqrt{2\pi}}\int e^{-\mathrm{i}kx}e^{-\mathrm{i}qx}\,\mathrm{d}x\\
&=\frac{1}{2\sqrt{2\pi}}\!\int\!e^{-\mathrm{i}(k-q)x}\,\mathrm{d}x+\frac{1}{2\sqrt{2\pi}}\!\int\!e^{-\mathrm{i}(k+q)x}\,\mathrm{d}x\\
&=\sqrt{\frac{\pi}{2}}\bigl(\delta(k-q)+\delta(k+q)\bigr).
\end{align}
