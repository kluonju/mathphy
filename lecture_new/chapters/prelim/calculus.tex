...
The statement $\lim_{x\to x_0}f(x)=L$ means that $f(x)$ approaches $L$ as $x$ approaches $x_0$. A function is \term{continuous} at $x_0$ if this limit exists and equals $f(x_0)$. Its \term{derivative} is defined by
\begin{equation}
...
An \term{antiderivative} $F$ of $f$ satisfies $F'=f$, so
\begin{equation}
...
\end{equation}
For variable bounds and a parameter-dependent integrand, the \term{Leibniz rule} is
\begin{align}
...
\end{equation}
for the \term{gradient}. If each $x_i$ depends on $t$, the chain rule becomes
\begin{equation}
\frac{\mathrm{d}f}{\mathrm{d}t}=\sum_{i=1}^n\frac{\partial f}{\partial x_i}\frac{\mathrm{d}x_i}{\mathrm{d}t}.
\end{equation}
When $f$ is sufficiently smooth near $x_0$, its \term{Taylor series} is
\begin{equation}
