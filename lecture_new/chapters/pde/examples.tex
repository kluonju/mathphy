...
More generally, a conserved density $\rho(\mathbf{x},t)$ with flux $\mathbf{J}$ satisfies the \term{continuity equation}
\begin{equation}
...
The scalar \term{wave equation} in $d$ spatial dimensions is
\begin{equation}
u_{tt}-c^2\nabla^2u=0.
\end{equation}
...
For a time-independent potential, solutions with a definite energy have the form $\psi(\mathbf{x},t)=\phi(\mathbf{x})e^{-\mathrm{i}Et/\hbar}$. Substitution gives the stationary Schr\"odinger eigenvalue problem
\begin{equation}
