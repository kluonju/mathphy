...
\end{equation}
The level curves $z(x,y)=\text{constant}$ obey $\mathrm{d}y/\mathrm{d}x=-z_x/z_y$. Therefore their slopes satisfy the \term{characteristic equation}
\begin{equation}
...
For a parabolic equation, $B^2-AC=0$, and the characteristic equation has the single root $\mathrm{d}y/\mathrm{d}x=B/A$. Use its characteristic family as $\xi$, then select a second coordinate $\eta$ which is not characteristic. Since $\xi_x/\xi_y=-B/A$, direct substitution gives
\begin{align}
...
u_{\alpha\alpha}+u_{\beta\beta}
=-\frac{1}{A_{12}}\left[(B_1+B_2)u_\alpha
+\mathrm{i}(B_2-B_1)u_\beta+2\widetilde C u+2\widetilde F\right].
\end{equation}
