...
The vibrating-string equation, first studied systematically by D'Alembert and others in the eighteenth century, is a prototype for a large class of PDEs. Consider a uniform, flexible string of length $\ell$, fixed at both ends and held taut. We seek the motion when it makes small transverse vibrations near its equilibrium position.
...
\end{enumerate}
We first neglect external forces. Place the fixed endpoints at $x=0$ and $x=\ell$, and let $u(x,t)$ be the transverse displacement at time $t$. For a fixed $t$, the graph of $u$ is the instantaneous shape of the string.
...
\qquad f=\frac{F}{\rho}.
\end{equation}
The wave equation is \term{hyperbolic}: changes propagate at finite speed and initial data are essential.

\subsection{The heat equation}

Consider heat conduction in a body $G$. Let $u(x,y,z,t)$ denote its temperature at position $(x,y,z)$ and time $t$. The local relation between temperature and heat flow is Fourier's experimental law. During a time interval $\mathrm{d}t$, the heat crossing an oriented surface element $\mathrm{d}S$ in the outward normal direction $\mathbf{n}$ is
\begin{equation}
...
\begin{equation}
\iiint_\Omega \rho c\,[u(x,y,z,t_2)-u(x,y,z,t_1)]\,\mathrm{d}V.
\end{equation}
...
\begin{equation}
\int_{t_1}^{t_2}\iiint_\Omega
\left[\rho c\,u_t-\nabla\cdot(k\nabla u)\right]\,\mathrm{d}V\,\mathrm{d}t=0.
\end{equation}
...
\label{eq:ch04-heat}
\end{equation}
This is the homogeneous \term{heat equation}. Diffusion smooths temperature differences rather than transporting a sharp disturbance at a fixed speed; this is reflected in the \term{parabolic} character of the equation.
...
\begin{equation}
\int_{t_1}^{t_2}\iiint_\Omega F(x,y,z,t)\,\mathrm{d}V\,\mathrm{d}t.
\end{equation}
...
Many physical systems evolve in time until forces, fluxes, or sources balance. In that stationary regime, time derivatives disappear and an \term{elliptic} equation remains. The two central stationary field equations in three dimensions are
\begin{equation}
\nabla^2u=0
\qquad\text{and}\qquad
\nabla^2u=f.
...
\end{equation}
The first is \termidx{Laplace's equation}{Laplace's equation}; its solutions are called harmonic functions. The second is \termidx{Poisson's equation}{Poisson's equation}, in which $f$ represents a source. In Cartesian coordinates,
\begin{equation}
...
V(x)=\frac{V_0}{d}x,
\qquad
\mathbf{E}=-\frac{V_0}{d}\,\mathbf{e}_x.
\end{equation}
