...
As a second illustration take a forced string on $[0,l]$ with free ends and vanishing initial data,
\begin{equation}
\begin{gathered}
u_{tt}-a^2 u_{xx}=A\cos\frac{\pi x}{l}\sin\omega t,\\
u_x\big|_{x=0}=u_x\big|_{x=l}=0,
\qquad
u\big|_{t=0}=u_t\big|_{t=0}=0.
\end{gathered}
\end{equation}
...
\sin\frac{n\pi a(t-\tau)}{l}
\cos\frac{n\pi\xi}{l}\cos\frac{n\pi x}{l}.
\end{aligned}
\end{equation}
Inserting into $u=\int_0^t\int_0^l f\,G\,\mathrm{d}\xi\,\mathrm{d}\tau$ and using orthogonality retains only the $n=1$ mode:
\begin{equation}
u(x,t)
=\frac{Al}{\pi a}\frac{1}{\omega^2-\pi^2 a^2/l^2}
\Bigl(\omega\sin\frac{\pi a t}{l}-\frac{\pi a}{l}\sin\omega t\Bigr)
\cos\frac{\pi x}{l}.
\end{equation}
When $\omega=\pi a/l$ the denominator vanishes and a resonant solution with secular growth must be constructed separately.
