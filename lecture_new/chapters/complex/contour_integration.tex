...
Let $\ell$ be an oriented curve from $z_0$ to $z_n$, partitioned at points $z_k$. The \term{contour integral} is the limit
\begin{equation}
\int_\ell f(z)\,\mathrm{d}z=\lim_{n\to\infty}\sum_{k=1}^n f(\zeta_k)(z_k-z_{k-1}).
\end{equation}
For $f=u+\mathrm{i}v$ and $\mathrm{d}z=\mathrm{d}x+\mathrm{i}\mathrm{d}y$, this is two real line integrals:
\begin{equation}
\int_\ell f(z)\,\mathrm{d}z=\int_\ell(u\,\mathrm{d}x-v\,\mathrm{d}y)+\mathrm{i}\int_\ell(v\,\mathrm{d}x+u\,\mathrm{d}y).
\end{equation}
...
Evaluate $\int z^2\,\mathrm{d}z$ from $0$ to $2+\mathrm{i}$. Along the straight path $z=(2+\mathrm{i})t$,
\begin{equation}
\int_0^1(2+\mathrm{i})^3t^2\,\mathrm{d}t=\frac{(2+\mathrm{i})^3}{3}
=\frac23+\frac{11}{3}\mathrm{i}.
\end{equation}
Along the broken path $0\to2\to2+\mathrm{i}$, the two contributions are
\begin{equation}
\int_0^2x^2\,\mathrm{d}x=\frac83,
\qquad
\int_0^1(2+\mathrm{i}y)^2\mathrm{i}\,\mathrm{d}y=-2+\frac{11}{3}\mathrm{i},
\end{equation}
which give the same result. This agrees with the antiderivative $z^3/3$.
\end{example}

\begin{theorem}[Cauchy's integral theorem]
\idx{Cauchy's integral theorem}
...
\begin{equation}
\left(\oint_{\ell_{\mathrm{out}}}+\int_{AB}+\oint_{\ell_{\mathrm{in}}}+\int_{B'A'}\right)f(z)\,\mathrm{d}z=0.
\end{equation}
As the two cut segments coalesce, their opposite orientations cancel. Hence, when all contours are written counterclockwise,
\begin{equation}
\oint_{\ell_{\mathrm{out}}}f(z)\,\mathrm{d}z=\sum_j\oint_{\ell_j}f(z)\,\mathrm{d}z.
\end{equation}
...
\begin{equation}
\oint_{|\zeta-z|=r}\frac{f(\zeta)}{\zeta-z}\,\mathrm{d}\zeta
=\mathrm{i}\int_0^{2\pi}f(z+re^{\mathrm{i}\theta})\,\mathrm{d}\theta
...
For every integer $n$,
\begin{equation}
\oint_C(z-a)^n\,\mathrm{d}z=\begin{cases}2\pi\mathrm{i},&n=-1\text{ and }a\text{ is inside }C,\\0,&\text{otherwise}.
\end{cases}
\end{equation}
...
\begin{equation}
\oint\frac{e^z}{z^n}\,\mathrm{d}z=\begin{cases}
