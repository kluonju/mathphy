...
\end{equation}
Around an isolated singularity the more general \term{Laurent series} is required:
\begin{equation}
f(z)=\sum_{k=-\infty}^{\infty}a_k(z-z_0)^k.
\end{equation}
The nonnegative powers form the analytic part and the negative powers the principal part. A singularity is \term{removable} when the principal part vanishes, a \term{pole} when it has finitely many terms, and \term{essential} when it has infinitely many terms. The coefficient $a_{-1}$ is the \term{residue}.
...
\frac{f(\zeta)(z-z_0)^k}{(\zeta-z_0)^{k+1}}\,\mathrm{d}\zeta\nonumber\\
