...
\begin{equation}
\int_{-\infty}^{\infty}f(x)\,\mathrm{d}x=2\pi\mathrm{i}\sum_{\operatorname{Im}z_j>0}\operatorname{Res}(f;z_j).
\end{equation}
...
\begin{equation}
\int_0^\infty\frac{\cos(mx)}{x^2+a^2}\,\mathrm{d}x=\frac{\pi}{2a}e^{-am}.
\end{equation}
...
\begin{equation}
\int_{-\infty}^{\infty}f(x)e^{\mathrm{i}mx}\,\mathrm{d}x
...
\begin{align}
\left|\int_{C_R}f(z)e^{\mathrm{i}mz}\,\mathrm{d}z\right|
&\leq2\varepsilon R\int_0^{\pi/2}e^{-mR\sin\theta}\,\mathrm{d}\theta\nonumber\\
&\leq2\varepsilon R\int_0^{\pi/2}e^{-2mR\theta/\pi}\,\mathrm{d}\theta
<\frac{\pi\varepsilon}{m}.
\end{align}
This proves Jordan's lemma. If $F$ is even, then
\begin{equation}
\int_0^\infty F(x)\cos(mx)\,\mathrm{d}x
=\frac12\int_{-\infty}^{\infty}F(x)e^{\mathrm{i}mx}\,\mathrm{d}x;
\end{equation}
the analogous statement holds for odd sine integrals.

Parameter differentiation provides a complementary method. From
\begin{equation}
\int_0^\infty e^{-ax}\cos(bx)\,\mathrm{d}x=\frac{a}{a^2+b^2},
\end{equation}
differentiation with respect to $a$ gives
\begin{equation}
\int_0^\infty xe^{-ax}\cos(bx)\,\mathrm{d}x=\frac{a^2-b^2}{(a^2+b^2)^2}.
\end{equation}
Adding a damping factor to $\int_0^\infty\sin x/x\,\mathrm{d}x$ and removing it at the end gives the classic result $\pi/2$. A keyhole contour gives the beta-integral form
\begin{equation}
\int_0^\infty\frac{x^{\alpha-1}}{1+x}\,\mathrm{d}x=\frac{\pi}{\sin(\pi\alpha)},\qquad 0<\alpha<1.
\end{equation}