...
\qquad z^s=e^{s\log z}.
\end{equation}
A \term{branch point} is a point around which continuation changes the value of a function. For $z^{1/2}$, a circuit about the origin exchanges the two values. A \term{branch cut} prevents such a circuit and defines a single-valued branch.
