...
\begin{equation}
\int_0^\infty\frac{x^{\alpha-1}}{1+x}\,\mathrm{d}x=\frac{\pi}{\sin(\pi\alpha)}.
\end{equation}
For the needed arc estimates, on the large circle $|z|=R$,
\begin{equation}
\left|\int_{C_R}\frac{z^{\alpha-1}}{1+z}\,\mathrm{d}z\right|
...
\begin{equation}
\int_0^\infty e^{\mathrm{i}x^2}\,\mathrm{d}x=(1+\mathrm{i})\sqrt{\frac{\pi}{8}},
\end{equation}
and so
\begin{equation}
...
\begin{equation}
\int_{C_R}e^{\mathrm{i}z^2}\,\mathrm{d}z=
\left.\frac{e^{\mathrm{i}z^2}}{2\mathrm{i}z}\right|_{C_R}
+\int_{C_R}\frac{e^{\mathrm{i}z^2}}{2\mathrm{i}z^2}\,\mathrm{d}z.
\end{equation}
...
Set
\begin{equation}
G(t)=\int_0^1\frac{x^t-1}{\log x}\,\mathrm{d}x.
\end{equation}
Then $G(0)=0$ and differentiation under the integral gives $G'(t)=\int_0^1x^t\,\mathrm{d}x=1/(t+1)$. Therefore
\begin{equation}
\int_0^1\frac{x^2-1}{\log x}\,\mathrm{d}x=G(2)=\int_0^2\frac{\mathrm{d}t}{t+1}=\log3.
\end{equation}
\end{example}

The same parameter philosophy can prove the Gaussian integral. Defining
\begin{equation}
I(t)=\int_0^\infty\frac{e^{-x^2}}{1+(x/t)^2}\,\mathrm{d}x
\end{equation}
