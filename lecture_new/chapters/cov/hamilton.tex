...
Newton's laws are often written in Cartesian coordinates, but a physical system may be described more naturally by angles, distances along constraints, or collective coordinates. Such variables are called \term{generalized coordinates} $q_i(t)$; they need not have dimensions of length. For generalized coordinates and velocities $\dot q_i(t)$, the \term{action} is
\begin{equation}
S[q]=\int_{t_1}^{t_2}L(q_i,\dot q_i,t)\,\mathrm{d}t.
\end{equation}
\termidx{Hamilton's principle}{Hamilton's principle} states that, among nearby paths with the same configurations at $t_1$ and $t_2$, the physical path satisfies
\begin{equation}
\delta S=0.
\end{equation}
...
\end{equation}
The first relation may agree with mechanical momentum in simple Cartesian systems, but canonical momentum is defined by the Lagrangian and can differ when velocity-dependent interactions or curvilinear coordinates are present. Provided the relations $p_i=\partial L/\partial\dot q_i$ can be inverted for the velocities, the \term{Hamiltonian} is a function of $(q_i,p_i,t)$ on phase space. Taking the differential and comparing coefficients of $
\mathrm{d}q_i$, $\mathrm{d}p_i$, and $\mathrm{d}t$ gives \termidx{Hamilton's equations}{Hamilton's equations}:
\begin{equation}
