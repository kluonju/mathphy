...
\Delta G_1=0.
\end{equation}
For the Dirichlet problem $G|_{\Sigma}=0$ one must have $G_1|_{\Sigma}=-G_0|_{\Sigma}$.

\paragraph{Three dimensions.}
Place the source at the origin and seek a radial solution of $\Delta G_0=0$ for $r\neq0$:
\begin{equation}
\frac{1}{r^2}\frac{\mathrm{d}}{\mathrm{d}r}\Bigl(r^2\frac{\mathrm{d}G_0}{\mathrm{d}r}\Bigr)=0
\implies
G_0=-\frac{C_1}{r}+C_2.
\end{equation}
Requiring $G_0\to0$ at infinity forces $C_2=0$. Integrating $\Delta G_0=\delta$ over a small ball about the origin gives $4\pi C_1=1$, so $C_1=1/(4\pi)$. Restoring a general source point,
\begin{equation}
\label{eq:G0-3d}
...
\begin{equation}
\Delta G=\delta(\mathbf{r}-\mathbf{r}_0),\qquad G\big|_{\text{sphere}}=0.
\end{equation}
...
For the disk $\rho<a$,
\begin{equation}
\Delta G=\delta(\mathbf{r}-\mathbf{r}_0),\qquad G\big|_{\rho=a}=0,
\end{equation}
...
Solve
\begin{equation}
\Delta_3 u=0\quad(r<a),\qquad u\big|_{r=a}=f(\theta,\varphi).
\end{equation}
...
Solve
\begin{equation}
\Delta_3 u=0\quad(z>0),\qquad u\big|_{z=0}=f(x,y).
\end{equation}
