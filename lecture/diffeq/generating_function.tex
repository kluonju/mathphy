...
与其导数 $f_n^{\prime}(x)$ 的关系。

Hermite 多项式 $H_n(x)$（常规缩放下）的生成函数为
...
$$
\frac{\partial}{\partial t} e^{-t^2+2 t x}=(2 x-2 t) e^{-t^2+2 t x}=
\sum_{n=0}^{\infty} n H_n(x) \frac{t^{n-1}}{n!} .
$$
...
式\eqref{eq:hermite-recur}称为递推公式；它允许从初始值（如 $H_0$ 和 $H_1$，可直接计算）构造整个 $H_n$ 序列。

对等式\eqref{eq:hermite-gen}关于 $x$ 求导，得导数公式：
