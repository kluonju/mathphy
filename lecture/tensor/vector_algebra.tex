% Vector Algebra section (中文翻译，基于 vector_analysis.pdf 第 1-2 节)
\section{向量代数}
...
\subsection{若干例题（保留原文例题结构）}
	extbf{例 1-1（向量加减）} 给定
\[
\mathbf{A}=4\mathbf{i}_x+4\mathbf{i}_y,\qquad \mathbf{B}=\mathbf{i}_x+8\mathbf{i}_y.
\]
求和与差以及它们的模长。
...
\]
	...
	extbf{例 1-2（点积求角度）} 例题给出
\[
\mathbf{A}=\sqrt{3}\,\mathbf{i}_x+\mathbf{i}_y,\qquad \mathbf{B}=2\mathbf{i}_x.
\]
求两向量间的夹角 $\theta$。

解：先计算各自的模长
\[
|\mathbf{A}|=\sqrt{(\sqrt{3})^2+1^2}=\sqrt{3+1}=2,\qquad |\mathbf{B}|=\sqrt{(2)^2}=2.
\]
再用点积的分量表达式
\[
...
\]
	...
	extbf{例 1-3（叉积与单位法向量）} 例题给出
\[
\mathbf{A}=-\mathbf{i}_x+\mathbf{i}_y+\mathbf{i}_z,\qquad \mathbf{B}=\mathbf{i}_x-\mathbf{i}_y+\mathbf{i}_z.
\]
...
计算各个子式：
\[
\begin{vmatrix}1&1\\ -1&1\end{vmatrix}=1\cdot 1-1\cdot(-1)=1+1=2,\qquad
...
两向量的模长：
\[
|\mathbf{A}|=\sqrt{(-1)^2+1^2+1^2}=\sqrt{3},\qquad |\mathbf{B}|=\sqrt{1^2+(-1)^2+1^2}=\sqrt{3}.
\]
因此
\[
...
% End of worked examples for 1-2

% （以上为 1-2 节的中文 LaTeX 表述，内容与 PDF 中的 1-2 节保持一致，若需包含全部逐字英文原文或更多例题的详细解答，我可以继续把每个例题完整抄入并解答。）
