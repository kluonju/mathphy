...
![路径上的线积分：分段 \\(\\mathbf{F}\\cdot\\mathrm{d}\\boldsymbol{\\ell}\\)](../figures/vec_line_integral.png)
...
取 \\(f=x^2 y\\)，验证从原点到 \\(P=(x_0,y_0)\\) 沿不同路径有

\\[
\int \nabla f\cdot\mathrm{d}\boldsymbol{\ell}=f(P)-f(O)=x_0^2 y_0.
\\]

**解.** \\(\nabla f=2 x y\,\mathbf{i}_x+x^2\,\mathbf{i}_y\\)。

- 路径 1：先沿 \\(x\\) 轴到 \\((x_0,0)\\)，再竖直到 \\(P\\)。第一段 \\(y=0\\) 故积分为零；第二段 \\(x=x_0\\)、\\(\mathrm{d}\boldsymbol{\ell}=\mathrm{d}y\,\mathbf{i}_y\\)，得 \\(\int_0^{y_0} x_0^2\,\mathrm{d}y=x_0^2 y_0\\)。
- 路径 2：先竖直再水平，同样得到 \\(x_0^2 y_0\\)。
- 路径 3：直线 \\(y=(y_0/x_0)x\\)，代入后积分仍得 \\(x_0^2 y_0\\)。
