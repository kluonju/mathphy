...
- [(b)] \\(f=ar^2\sin\phi+brz\cos2\phi\\)（圆柱坐标）;

- [(c)] \\(f=a+br\sin\theta\cos\phi\\)（球坐标，注意题中 \\(r\\) 在分母处的项）。

\paragraph{解}
(a) 在直角坐标下逐分量求偏导：

\\[
\frac{\partial f}{\partial x}=2ax y,   \frac{\partial f}{\partial y}=ax^2+2by z,   \frac{\partial f}{\partial z}=by^2.
\\]
...
\frac{\partial f}{\partial r}=2ar\sin\phi+b z\cos2\phi,
   \frac{\partial f}{\partial\phi}=ar^2\cos\phi-2br z\sin2\phi,
   \frac{\partial f}{\partial z}=br\cos2\phi.
\\]
...
\frac{\partial f}{\partial r}=b\sin\theta\cos\phi,
   \frac{\partial f}{\partial\theta}=br\cos\theta\cos\phi,
   \frac{\partial f}{\partial\phi}=-br\sin\theta\sin\phi.
\\]
...
（注：最后两项可分别简化为 \\(b\cos\theta\cos\phi \mathbf{i}_\theta/r\\) 与 \\(-b\sin\phi \mathbf{i_\phi/(r\sin\theta)\\)；上式保留形式用于逐项展示。）
