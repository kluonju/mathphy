...
\\[
f(t)=\frac{1}{2 \pi \mathrm{i}} \int_{\sigma-\mathrm{i} \infty}^{\sigma+\mathrm{i} \infty} \bar{f}(p) e^{ p t} dp,
\\]
...
**推广的约当引理** 设 \\(C_R\\) 是以 \\(p=0\\) 为圆心, 以 \\(R\\) 为半径的圆周在直线 \\(\Re p=a(>0)\\) 左侧的圆弧. 若当 \\(|p| \rightarrow \infty\\) 时, \\(\bar{f(p)\\) 在 \\(\frac{\pi{2}-\delta \leqslant \operatorname{Arg} p \leqslant \frac{3}{2} \pi+\delta\\) 中一致趋于零 ( \\(\delta\\) 是小于 \\(\frac{\pi{2}\\) 的任意正数), 则

\\[
\lim _{R \rightarrow \infty} \int_{C_R} \bar{f}(p) e^{p t} d p=0  (t>0) .
\\]
...
\\[
f(t)=\int_{a-\mathrm{i} \infty}^{a+\mathrm{i} \infty} \frac{e^{p t}}{\sqrt{p}} d p=-\frac{1}{2 \pi \mathrm{i}} \lim _{R \rightarrow \infty}\left(\int_{l_1}+\int_{l_2}\right) \frac{e^{p t}}{\sqrt{p}} d p .
\\]

\\(p\\) 在割线下沿 \\(l_1\\) 和上沿 \\(l_2\\) 上的辐角分别为 \\(-\pi\\) 和 \\(\pi\\). 这样, 在 \\(l_1\\) 上, \\(\sqrt{p}=\sqrt{|p|} e^{-\mathrm{i} \pi / 2}=\\) \\(-\mathrm{i} \sqrt{|p|}\\); 在 \\(l_2\\) 上 \\(\sqrt{p}=\sqrt{|p|} e^{\mathrm{i} \pi / 2}=\mathrm{i} \sqrt{|p|}\\). 于是,

\\[
\begin{aligned}
f(t) & =-\frac{1}{2 \pi \mathrm{i}} \int_0^{-\infty} e^{p t} \frac{1}{-\mathrm{i} \sqrt{|p|}} d p-\frac{1}{2 \pi \mathrm{i}} \int_{-\infty}^0 e^{p t} \frac{1}{\mathrm{i} \sqrt{|p|}} d p \\\\
...
\\[
\begin{aligned}
\frac{e^{-\alpha p}}{p(p+b)} &  \Leftrightarrow  \int_0^t \Theta(\tau-\alpha) e^{-b(t-\tau)} d \tau \\\\
